...
boolean Adafruit_SGP30::begin(TwoWire *theWire, boolean initSensor) {
  ...
  command[0] = 0x36;
  command[1] = 0x82;
  if (!readWordFromCommand(command, 2, 100, serialnumber, 3)) {
  ...
  command[0] = 0x20;
  command[1] = 0x2F;
  if (!readWordFromCommand(command, 2, 100, &featureset, 1)) {
    Serial.println("FS read fail");
    //return false;
  }
  Serial.print("Featureset 0x"); Serial.println(featureset, HEX);
  if ((featureset & 0xF0) != SGP30_FEATURESET) {
    Serial.println("FS match fail");
    //return false;
  }
